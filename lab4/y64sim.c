...
mem_t *init_mem(int len)
{
    ...
    len = ((len+BLK_SIZE-1)/BLK_SIZE)*BLK_SIZE;//round up so that len is multiple of 32
    m->len = len;
...
mem_t *dup_mem(mem_t *oldm)//duplicate memory area
{
...
long_t get_reg_val(mem_t *r, regid_t id)//r is used to store all registers
{
    long_t val = 0;
    if (id >= REG_NONE)
        return 0;
