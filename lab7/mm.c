/*
 * mm.c - a simple allocator based on an explicit free list with immediate boundary-tag coalescing
 * 
 * In this approach, an explicit free list is used to organize blocks. 
 * Each block has a header and a footer identical to the textbook, containing information about the size of the block and its allocation.
 * Besides, each free block has an 8-byte predecessor pointer placed right after the header, followed by a successor pointer,
 * storing the previous and the next free block in the free list separately.
 * Those pointers form a double linked list as the free list, connnecting all free blocks.
 * The allocator is aligned to 8, using LIFO-ordering, first-fit and immediate coalescing, and its minimum block size is 6 words due to the boundary tags and pointers.
 *
 * The allocator initialize the heap by putting a padding word and a prologue block at the beginning and a epilogue block at the end, then put a free chunk in between.
 * On allocation, the allocator first select a free block in the free list using first-fit, split it if necessary, then set the selected block to allocated by changing its header and footer.
 * If the allocator split the block, it will coalesce the rest part if necessary, then add it to the beginning of the free list.
 * To free a block, the allocator simply set its tags and pointers, then coalesce it and add it to the beginning of the free list.
 * The allocator will extend the heap by calling mem_sbrk, adding a new free block to the free list every time it finds there is no free block big enough.
 *
 * By Ruiyang Jin at 3/18/2018
 */
#include <stdio.h>
#include <stdlib.h>
#include <assert.h>
 ...
 * Then it set heap_listp to the prologue block, free_listp to NULL, and extend the heap.
